...
NativenessQnoAln::NativenessQnoAln()
{
    Name("NativenessQnoAln");
    fixed_his=true;
    altcrd="first";
}
...
/**
\page opt_NativenessQnoAln Nativeness Q parameter
...
59 in that file. The NativenessQnoAln %Observable obeys the same rules for
...
\sa prf::NativenessQnoAln
*/
int NativenessQnoAln::init_obs()
{
...
void NativenessQnoAln::his_range(double xmn, double xmx)
{
    xmin0=0; xmax0=1; //no matter what anyone tries to set these to.
    xbin0=0.01;
}

int NativenessQnoAln::init()
{
    ...
    if (pdb.read_matrix()==0) {
        ...
        <<"NativenessQnoAln initialization failed.\n";
        return 0;
    }
...
double NativenessQnoAln::evaluate()
{
    double obvl=0;
    Shape curshp;
...
void NativenessQnoAln::loc_val(size_t i, int & resi, double &qi)
{
    if (i>0 && i< Qloc.size()) {
        qi=Qloc[i];
        resi=res[i];
...
void NativenessQnoAln::avg_fill(int i)
{
...
void NativenessQnoAln::avg_reset()
{
...
void NativenessQnoAln::avg_write(Output &op)
{
