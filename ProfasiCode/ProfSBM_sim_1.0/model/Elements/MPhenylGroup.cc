...
double MPhenylGroup::ringbl=1.39;
double MPhenylGroup::bXH=1.0;

MPhenylGroup::MPhenylGroup()
{
    th01=acos(-1.0/3);
    bstm=bprm=1.52;

    for (int i=0;i<5;++i) brnc[i]=bXH;

    nm="Modified Phenyl Group";
...
void MPhenylGroup::Initialize()
{
    hexarm.Initialize(bprm,bstm,ringbl,pi-th01,pi/3);
}

void MPhenylGroup::Create()
{
    ...
    atm[8].Pos(atm[3].Position()-(brnc[0]/ringbl)*hexv2);
    atm[9].Pos(atm[4].Position()+(brnc[1]/ringbl)*hexv1);
    atm[10].Pos(atm[5].Position()+(brnc[2]/ringbl)*hexv0);
    atm[11].Pos(atm[6].Position()+(brnc[3]/ringbl)*hexv2);
    atm[12].Pos(atm[7].Position()-(brnc[4]/ringbl)*hexv1);
}

void MPhenylGroup::ExportConnections(ConnectionsMatrix &aa)
{
...
void MPhenylGroup::BuildConnections()
{
    for (int si=0;si<nsubnd;++si) {
        int subbase=0;
        int k=subnode[si]->ATOM(1).UniqueId();

        for (int j=2;j<8;++j) if (atm[j].UniqueId()==k) {subbase=j;break;}

        subnode[si]->SetBase(atm[subbase-1]);

        if (subbase!=7) {
            subnode[si]->SetBase(atm[subbase+1]);
        } else subnode[si]->SetBase(atm[2]);

        subnode[si]->BuildConnections();
    }
}

void MPhenylGroup::LocPairs(deque<pair<int,int> > & lcp)
{
    for (int i=0;i<naltbases;++i) {
...
void MPhenylGroup::SetBondLengths(double x0,double x1,double x2,
                                  double x3,double x4)
{
    bprm=x0;bstm=x1;
}

void MPhenylGroup::SetBondAngles(double x0,double x1,double x2,double x3)
{
    th01=x0;
}

void MPhenylGroup::SetBranchLength(int i, double x)
{
    brnc[i%5]=x;
}
