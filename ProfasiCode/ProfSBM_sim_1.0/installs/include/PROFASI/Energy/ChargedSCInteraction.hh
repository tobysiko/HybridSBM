...
#ifndef ChargedSCInteraction_HH
#define ChargedSCInteraction_HH
#include "Energy.hh"
...
//! Effective hydrophobic attraction between non-polar side chains
    ...
    class ChargedSCInteraction:public Energy
    {
    public:
        ChargedSCInteraction();
        ~ChargedSCInteraction();
        void init();
        double evaluate();
        ...
        inline double contribution(int i, int j) const {
            return sclfct*Mehp[NCAA * i + j];
        }

        inline int ncaa() const {
            return NCAA;
        }
    ...
    private:
        ...
        double cg_pair(int i, int j,double(*distf)(int,int));
        double dcg_pair(int i, int j, bool perhint,
        ...
        std::valarray<int> natom, jh, ih, nh, chainof;
        double r2min[12];
        int r2minpartner[12];
        std::valarray <OneLetterCode> seq;
        static const int nCHgrp;
        static const double hpstr[];
        static std::string hatms[][4];
