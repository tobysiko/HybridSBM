...
if __name__=="__main__":
	...
	if True: # HANDLING COMMAND LINE OPTIONS...
		...
		op.add_argument('-s','--inPDBFilename', action="store", type="string", dest="inPDBFilename",help="structure file in PDB format. Comma-separate multiple files in order to use potential type FMULTIGAUSS and omit -S option.")
		op.add_argument('-c','--contactType', action="store", type="string", dest="contactType",help="type of contact definition: CUTOFF, SHADOW, HYBRID, RESIDUE  (%default)")
		op.add_argument('-a','--contactAtoms', action="store", type="string", dest="contactAtoms",help="list of atom types for native contacts, e.g. CA,CB,C,N,O  (%default)")
		op.add_argument('-n','--chainSep', action="store", type="int", dest="chainSep",help="min number of residues between contacting atoms  (%default)")
		op.add_argument('-d','--distCutoff', action="store", type="float", dest="distCutoff",help="distance cutoff for defining contacts (%default Ang)")
		op.add_argument('-m','--wellDepth', action="store", type="float", dest="wellDepth",help="well depth at native distance (%default)")
		op.add_argument('-w','--wellWidth', action="store", type="float", dest="wellWidth",help="GAUSS/DUALGAUSS: width of well (%default)")
		op.add_argument('-r','--repulDist', action="store", type="float", dest="repulDist",help="GAUSS/DUALGAUSS: excluded volume radius.  0=no repulsion; -x=radius is min distance per contact - x (FMULTIGAUSS)  (%default)")
		op.add_argument('-p','--repulSteep', action="store", type="float", dest="repulSteep",help="GAUSS/DUALGAUSS: steepness of repulsion (%default)")
		op.add_argument('-e','--ensembleCutoff', action="store", type="float", dest="ensembleCutoff",help="when PDB file contains multiple models, retain contacts present in this fraction of models (%default)")
		op.add_argument('-N','--normEnergy', action="store",type="float", dest="normEnergy",help="normalize contact energies to given amount per structure. Overrides options m and M. (%default=don't normalize)")
		op.add_argument('--gmxpath', action="store", type="string", dest="gmxpath",help="path to Gromacs executables (SHADOW)")
		op.add_argument('--scmpath', action="store", type="string", dest="scmpath",help="path to SCM.jar  (SHADOW)")
		op.add_argument('-L','--label',action='store', type="string",dest="label",help="string that will be appended to the keyword 'DistanceRestraint' in the Profasi energy output")
		op.add_argument('-S','--seqFromModel',action='store', type="int",dest="seqFrom",help="model number to select AA sequence for contacts. By default: take first model")
		op.add_argument('-x','--nonNative',action='store_true',dest="includeNonNative",help="include non-native contacts")
		op.add_argument('-R','--range',action='store',type="string",dest="selectedRange",help="specify residues ranges as strings,e.g. '1-20,29,30,42-56' add 'x' at the beginning to exclusively only consider these residues, otherwise contacts involving only one of these residues will be considered")
		op.add_argument('-M','--smoothGaussian',action='store_true',dest="smoothGaussian",help='set function to epsilon if between min and max distance')
		op.add_argument('-l','--MSA',action='store',type='string',dest='MSA_file',help="filename of Multiple Sequence Alignment in a format read by BioPython. Will generate compatible contact lists for each aligned sequence.")
		op.add_argument('-v','--verbosity',action="store",type="int",dest="verbosity",help="0: minimal output, 1: important output, 2: debugging")
		op.add_argument('-C','--nchains',action="store",type="int",dest="nchains",help="number of identical chains")
		op.add_argument('-H','--removeSharedFrom', action="store", type="string", dest="removeSharedFrom", help="PROFASI contact file. All contacts present in this file will be removed from the output contacts.")
		op.add_argument('-i','--intersectWith', action="store", type="string", dest="intersectWith", help="PROFASI contact file. All contacts NOT present in this file AND the regular input file (-s) will be removed from the output contacts.")
		
		op.add_argument('-D', '--homodimer', action='store', type="string", dest="dimerfile", help="PDB file containing a homodimer of the protein")
		op.add_argument('-I','--indegree',action='store_true',dest="calcContactIndegree",help='calculate indegree of amino acids in contact network (i.e. how many contacts per amino acid)')
		op.add_argument('-f','--chain',action="store",type="string",dest="defaultChain",help="single chain to use. defaults to A")
